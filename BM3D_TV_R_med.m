function [SNR, PSNR, y_est, y_hat] = BM3D_TV_R_med(y, z, sigma, profile, print_to_screen)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if dump_output_information == 1,
    fprintf('TV-BM3D, Image: %s (%dx%d), sigma: %.1f\n', image_name, size(z,1), size(z,2), sigma);
end
...
n = 3;
m = 3;
y_hat = medfilt2(z, [n m]);
% y_hat = TV_Denoising(z, y);%NoLambda_natural(z, y);%
y_hat = single(y_hat);
% bm3d_thr(z, hadper_trans_single_den, Nstep, N1, N2, lambda_thr2D,...
...
if dump_output_information == 1,
    SNR_INITIAL_ESTIMATE = 20*log10(norm(y(:))/norm(y(:)-y_hat(:)));
    fprintf('BASIC ESTIMATE, SNR: %.2f dB\n', SNR_INITIAL_ESTIMATE);
end
...
SNR = 0; %% Remains 0 if the true image y is not available
if (~y_is_invalid_image), % checks if y is a valid image
    SNR = 20*log10(norm(y(:))/norm(y(:)-y_est(:))); % y is valid
end
...
if dump_output_information == 1,
    ...
    fprintf('FINAL ESTIMATE (total time: %.1f sec), SNR: %.2f dB\n', ...
        wiener_elapsed_time + estimate_elapsed_time, SNR);
